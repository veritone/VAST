...
class TokenMasker(nn.Module):
    ...
    def perform_mask(self, tokens, mask_prob):
        ...
        labels = (
            -np.ones(tokens.shape, dtype=np.int64) * 100
        )  ### -100 ignore idx for nn.CrossEntropyLoss used in BERT
...
class MMGeneralModule(nn.Module):
    ...
    def modify_checkpoint(self, checkpoint):
        ...
        if self.config.frame_embedding_type == "adaptive":
            if "vision_frame_embedding" in checkpoint:
                ...
                if pretrain_embed.shape[1] != self.config.max_vision_sample_num:
                    pretrain_embed = F.interpolate(
                        pretrain_embed.permute(0, 2, 1),
                        self.config.max_vision_sample_num,
                        mode="nearest",
                    ).permute(0, 2, 1)
                ...
                if pretrain_embed.shape[1] != self.config.max_vision_sample_num:
                    ...
                    checkpoint["vision_perceiver.vision_frame_embedding"] = (
                        pretrain_embed
                    )
            ...
            if "audio_frame_embedding" in checkpoint:
                ...
                if pretrain_embed_a.shape[1] != self.config.max_audio_sample_num:
                    pretrain_embed_a = F.interpolate(
                        pretrain_embed_a.permute(0, 2, 1),
                        self.config.max_audio_sample_num,
                        mode="nearest",
                    ).permute(0, 2, 1)
        ...
        if self.config.vision_encoder_type.startswith("clip"):
            vision_width = checkpoint[
                "vision_encoder.visual.positional_embedding"
            ].shape[1]
            vision_layers = len(
                [
                    k
            ...
            vision_patch_size = checkpoint["vision_encoder.visual.conv1.weight"].shape[
                -1
            ]

            grid_size = round(
                (checkpoint["vision_encoder.visual.positional_embedding"].shape[0] - 1)
                ** 0.5
            )
            ...
            if new_grid_size != grid_size:
                src_oth = F.interpolate(
                    src_oth.reshape(grid_size, grid_size, vision_width)
                    .permute(2, 0, 1)
                    .unsqueeze(0),
            ...
            vision_patch_size = checkpoint[
                "vision_encoder.visual.patch_embed.proj.weight"
            ].shape[-1]

            grid_size = round(
                (checkpoint["vision_encoder.visual.pos_embed"].shape[1] - 1) ** 0.5
            )
    ...
    def construct_vision_encoder(self):
        ...
        if self.config.vision_encoder_type.startswith(
            "clip"
        ) or self.config.vision_encoder_type.startswith("evaclip"):
    ...
    def load_videoswin_model(self):
        ...
        time_stride = 1
        self.vision_encoder = SwinTransformer3D(
            time_stride=1,
            embed_dim=128,
            num_heads=[4, 8, 16, 32],
        ...
        videoswin_weight = torch.load(
            "./pretrained_weights/videoswin_base_k600_22k.pth", map_location="cpu"
        )["state_dict"]
        videoswin_weight = {
            k.replace("backbone.", ""): v for k, v in videoswin_weight.items()
        }
        missing_keys, unexpected_keys = self.vision_encoder.load_state_dict(
            videoswin_weight, strict=False
        )
    ...
    def load_ast_model(self):
        ...
        ast_weight = torch.load(
            "./pretrained_weights/audioset_10_10_0.4593.pth", map_location="cpu"
        )
        audio_weight = {}
        audio_weight["audio_embeddings.cls_token"] = ast_weight["module.v.cls_token"]
        audio_weight["audio_embeddings.distill_token"] = ast_weight[
            "module.v.dist_token"
        ]
        audio_weight["audio_embeddings.first_conv.weight"] = ast_weight[
            "module.v.patch_embed.proj.weight"
        ]  ### need to permute?
        audio_weight["audio_embeddings.first_conv.bias"] = ast_weight[
            "module.v.patch_embed.proj.bias"
        ]
        ...
        pos_weight_oth = (
            pos_weight_oth.reshape(12, 101, -1).permute(2, 0, 1).unsqueeze(0)
        )
        ...
        pos_weight_oth = (
            F.interpolate(
                pos_weight_oth,
                size=(tar_patch_num_height, tar_patch_num_width),
                mode="bilinear",
            )
            .squeeze()
            .permute(1, 2, 0)
        ...
        for i in range(12):
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.0.weight"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.qkv.weight"][:768, :]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.0.bias"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.qkv.bias"][:768]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.1.weight"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.qkv.weight"][
                768 : 2 * 768, :
            ]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.1.bias"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.qkv.bias"][
                768 : 2 * 768
            ]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.2.weight"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.qkv.weight"][
                2 * 768 :, :
            ]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.2.bias"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.qkv.bias"][2 * 768 :]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.3.weight"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.proj.weight"]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".attention.linears.3.bias"
            ] = ast_weight["module.v.blocks." + str(i) + ".attn.proj.bias"]
            audio_weight[
                "audio_encoder.layer." + str(i) + ".ff_layer.linear1.weight"
            ] = ast_weight["module.v.blocks." + str(i) + ".mlp.fc1.weight"]
            audio_weight["audio_encoder.layer." + str(i) + ".ff_layer.linear1.bias"] = (
                ast_weight["module.v.blocks." + str(i) + ".mlp.fc1.bias"]
            )
            audio_weight[
                "audio_encoder.layer." + str(i) + ".ff_layer.linear2.weight"
            ] = ast_weight["module.v.blocks." + str(i) + ".mlp.fc2.weight"]
            audio_weight["audio_encoder.layer." + str(i) + ".ff_layer.linear2.bias"] = (
                ast_weight["module.v.blocks." + str(i) + ".mlp.fc2.bias"]
            )
            audio_weight["audio_encoder.layer." + str(i) + ".layernorm1.weight"] = (
                ast_weight["module.v.blocks." + str(i) + ".norm1.weight"]
            )
            audio_weight["audio_encoder.layer." + str(i) + ".layernorm1.bias"] = (
                ast_weight["module.v.blocks." + str(i) + ".norm1.bias"]
            )
            audio_weight["audio_encoder.layer." + str(i) + ".layernorm2.weight"] = (
                ast_weight["module.v.blocks." + str(i) + ".norm2.weight"]
            )
            audio_weight["audio_encoder.layer." + str(i) + ".layernorm2.bias"] = (
                ast_weight["module.v.blocks." + str(i) + ".norm2.bias"]
            )
        audio_weight["audio_encoder.last_layernorm.weight"] = ast_weight[
            "module.v.norm.weight"
        ]
        audio_weight["audio_encoder.last_layernorm.bias"] = ast_weight[
            "module.v.norm.bias"
        ]
    ...
    def load_clip_model(self):
        if self.config.vision_encoder_type.startswith("evaclip"):
            ...
            if self.config.vision_encoder_type == "clip_vit_base_16":
                clip_weight = torch.jit.load(
                    "./pretrained_weights/clip/ViT-B-16.pt", map_location="cpu"
                )
                self.vision_dim = 768
            elif self.config.vision_encoder_type == "clip_vit_large_14_336px":
                clip_weight = torch.jit.load(
                    "./pretrained_weights/clip/ViT-L-14-336px.pt", map_location="cpu"
                )
                self.vision_dim = 1024
            elif self.config.vision_encoder_type == "clip_vit_base_32":
                clip_weight = torch.jit.load(
                    "./pretrained_weights/clip/ViT-B-32.pt", map_location="cpu"
                )
                self.vision_dim = 768
            clip_weight = clip_weight.state_dict()

            self.vision_encoder = build_model(
                clip_weight, self.config.vision_resolution, self.config.checkpointing
            ).float()
    ...
    def forward_vision_encoder(self, vision_pixels):  ### b,n,3,h,w
        ...
        if self.config.vision_encoder_type.startswith(
            ...
            vision_output = self.vision_encoder.visual(
                vision_pixels.reshape(b * n, 3, h, w), return_all_features=True
            )
    ...
    def forward_audio_encoder(self, audio_spectrograms):
        if self.config.audio_encoder_type.startswith("ast"):
            audio_spectrograms = audio_spectrograms.permute(
                0, 1, 3, 2
            )  ### 1028,64 to 64,1028
            (
                b,
                ...
                h,
                w,
            ) = audio_spectrograms.shape
            audio_spectrograms = audio_spectrograms.reshape(
                -1, *audio_spectrograms.shape[2:]
            )
    ...
    def get_multimodal_forward_input_vision(self, vision_output):
        ...
        if self.config.frame_embedding_type == "adaptive":
            if n != self.vision_frame_embedding.shape[1]:  #### testing and interpolate
                ...
                vision_frame_embedding = (
                    F.interpolate(
                        self.vision_frame_embedding.float().permute(0, 2, 1),
                        n,
                        mode="nearest",
                    )
                    .permute(0, 2, 1)
                    .to(self.vision_frame_embedding)
                )
            else:
    ...
    def get_multimodal_forward_input_audio(self, audio_output):
        ...
        if n != self.audio_frame_embedding.shape[1]:  #### testing and interpolate
            audio_frame_embedding = F.interpolate(
                self.audio_frame_embedding.permute(0, 2, 1), n, mode="nearest"
            ).permute(0, 2, 1)
        else:
    ...
    def load_swin_model(self):
        ...
        if self.config.vision_encoder_type.startswith("swin_base_22k_224"):
            swin_config = get_config(
                "./pretrained_weights/swin/swin_base_patch4_window7_224_22k.yaml"
            )
            swin_weight = torch.load(
                "./pretrained_weights/swin/swin_base_patch4_window7_224_22k.pth",
                map_location="cpu",
            )["model"]
            self.vision_dim = 1024
        elif self.config.vision_encoder_type.startswith("swin_large_22k_224"):
            swin_config = get_config(
                "./pretrained_weights/swin/swin_large_patch4_window7_224_22k.yaml"
            )
            swin_weight = torch.load(
                "./pretrained_weights/swin/swin_large_patch4_window7_224_22k.pth",
                map_location="cpu",
            )["model"]
