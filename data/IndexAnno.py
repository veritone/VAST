    ...
    def __getitem__(self, i):
        ...
        if self.vision_mapper:
            if self.vision_mapper.vision_format == "video_feats":
                ...
                if (
                    vision_pixels is None
                ):  ###wrong img/video, resample when training and raise error when testing
                    if self.training:
        ...
        if self.audio_mapper:
            ...
            if (
                audio_spectrograms is None
            ):  ### wrong audio, resample when training and raise error when testing
                if self.training:
                    resample_idx = random.choice(self.idx)
                    LOGGER.info(
