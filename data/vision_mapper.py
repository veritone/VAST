...
class VisionMapper(object):
    def __init__(self, d_cfg, args):
        ...
        if args.model_cfg.vision_encoder_type.startswith(
            "clip"
        ) or args.model_cfg.vision_encoder_type.startswith("evaclip"):
    ...
    def read(self, id_):
        if self.vision_format.startswith("video"):
            if self.vision_format == "video_feats":
                ...
                idxs = (
                    torch.arange(0, num_pre_clips + 1, 1.0)
                    / num_pre_clips
                    * num_src_clips
                )
                ...
                try:
                    if self.vision_format == "video_rawvideo":
                        ...
                        vision_pixels = torch.from_numpy(
                            frames.transpose(0, 3, 1, 2) / 255.0
                        )  ### nX3xHxW
                        ...
                        if self.dense_extraction:  ### for feature extraction
                            sample_num = int(
                                len(frames) * self.extract_fps / self.frame_fps
                            )
